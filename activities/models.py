...
class Activity(models.Model):
    ...
    capacity = models.IntegerField("人数限制", default=100)
    ...
    def generate_checkin_code(self):
        """生成新的签到码"""
        self.checkin_code = ''.join(random.choices(string.digits + string.ascii_uppercase, k=6))
        self.save()
    ...
    def is_checkin_available(self):
        ...
        if not self.checkin_code:
            return False
        now = timezone.now()
        # 活动进行中才能签到（开始前30分钟到结束后2小时）
        start_time = self.date - timezone.timedelta(minutes=30)
        end_time = self.date + timezone.timedelta(hours=2)
...
class Registration(models.Model):
    ...
    # 签到日志
    checkin_log = models.TextField("签到日志", blank=True, null=True, help_text="记录签到操作的详细信息")
    ...
    def checkin(self, ip_address=None, user_agent=None):
        ...
        # 记录日志
        existing_log = json.loads(self.checkin_log or "[]")
        existing_log.append(log_entry)
        self.checkin_log = json.dumps(existing_log, ensure_ascii=False)
